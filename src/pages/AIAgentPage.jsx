        ...
        <Stack sx={{ height: '100%', overflow: 'hidden' }}>
            <Box sx={{ p: 2, borderBottom: '1px solid #e2e8f0' }}>
                ...
                <Button 
                    ...
                    variant="contained" 
                    sx={{ minWidth: isCollapsed ? 0 : 'auto', p: isCollapsed ? 1 : undefined, display: 'flex', justifyContent: 'center', alignItems: 'center' }}
                >
                    <AddIcon sx={{ mr: isCollapsed ? 0 : 1 }} />
                    {!isCollapsed && <span>New Research</span>}
                </Button>
            </Box>
            <Box sx={{ flexGrow: 1, overflowX: 'hidden', overflowY: 'auto', p: 2, display: 'flex', flexDirection: 'column', alignItems: isCollapsed ? 'center' : 'stretch' }}>
                {!isCollapsed && <Typography variant="caption" sx={{ textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700, color: 'text.secondary', display: 'block', mb: 1.5, whiteSpace: 'nowrap' }}>Recent Insights</Typography>}
                <Stack spacing={0.5} sx={{ mb: 3, alignItems: isCollapsed ? 'center' : 'stretch' }}>
                    {['Q3 Tech Sector Summary', 'SaaS Multiples 2024', 'Emerging Markets Growth'].map((item, i) => (
                        <Button key={i} fullWidth={!isCollapsed} onClick={onClose}
                            sx={{ 
                            ...
                            {!isCollapsed && <Typography variant="body2" noWrap>{item}</Typography>}
                        </Button>
                    ))}
                </Stack>
            </Box>
            <Box sx={{ p: 2, borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'center' }}>
                {!isCollapsed ? (
                    <Paper variant="outlined" sx={{ p: 2, width: '100%' }}>
                        <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 0.5 }}>Account Name</Typography>
                        <Typography variant="caption" color="primary" sx={{ fontWeight: 700, display: 'block', mb: 1 }}>Pro Plan</Typography>
                        <Typography variant="caption" sx={{ textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700, color: 'text.secondary', display: 'block', mb: 1 }}>Usage Limit</Typography>
                        <LinearProgress variant="determinate" value={65} sx={{ height: 4, borderRadius: 2, mb: 1 }} />
                        <Typography variant="caption" color="text.secondary">650 / 1000 requests</Typography>
                    </Paper>
                ) : (
                    <Box sx={{ width: 32, height: 32, borderRadius: '50%', bgcolor: 'primary.main', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 'bold', flexShrink: 0 }}>
                        A
                    </Box>
                )}
            </Box>
...
const InputBox = ({ inputValue, setInputValue, handleSend, hasChat }) => (
    <Stack component={motion.div} layout layoutId="search-box" transition={{ type: 'spring', bounce: 0.2, duration: 0.8 }} sx={{ maxWidth: 800, mx: 'auto', width: '100%', px: { xs: 1, md: 0 } }}>
        <Paper elevation={4} sx={{ 
            ...
            <TextField
                fullWidth multiline maxRows={8} minRows={2}
                ...
                slotProps={{ 
                    input: { 
                        ...
                        sx: { color: 'text.primary', fontSize: { xs: '1rem', md: '1.1rem' }, '&::placeholder': { color: 'text.secondary', opacity: 1 } }
                    } 
                }}
                sx={{ px: 1, mb: { xs: 1, md: 2 } }}
                onKeyDown={(e) => {
            ...
            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ px: 0.5 }}>
                <IconButton size="small" sx={{ color: 'text.secondary', '&:hover': { color: 'text.primary', bgcolor: 'rgba(0,0,0,0.04)' } }}>
                ...
                <Stack direction="row" gap={{ xs: 0.5, md: 1 }} alignItems="center">
                    ...
                    <IconButton onClick={handleSend} size="small" 
                        sx={{ 
                            color: inputValue.trim() ? '#fff' : 'text.secondary', 
                            bgcolor: inputValue.trim() ? 'primary.main' : 'transparent',
                            '&:hover': { 
                                color: inputValue.trim() ? '#fff' : 'text.primary',
                                bgcolor: inputValue.trim() ? 'primary.dark' : 'rgba(0,0,0,0.04)' 
                            } 
                        }}>
                        {inputValue.trim() ? <SendIcon fontSize="small" /> : <GraphicEqIcon fontSize="small" />}
                    </IconButton>
                </Stack>
            </Stack>
        </Paper>
        ...
        {hasChat && (
            <Typography variant="caption" color="text.secondary" textAlign="center" sx={{ mt: 1, fontSize: { xs: '0.6rem', md: '0.75rem' } }}>
        ...
        <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden', bgcolor: 'background.default' }}>
            ...
            <Box component="main" sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', position: 'relative', minWidth: 0 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 1.5, borderBottom: '1px solid #e2e8f0', bgcolor: 'background.paper' }}>
                    ...
                    <Typography variant="subtitle2" sx={{ fontFamily: '"League Spartan", sans-serif', fontWeight: 800 }}>Researcha AI</Typography>
                </Box>
                    ...
                    <>
                        <Box sx={{ flexGrow: 1, overflow: 'auto', p: { xs: 2, md: 3 } }}>
                            <Stack spacing={4} sx={{ maxWidth: 720, mx: 'auto' }}>
                                {/* User Message */}
                                <Box component={motion.div} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 200, damping: 20, delay: 0.1 }} sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                                ...
                                {/* AI Message */}
                                <Box component={motion.div} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 200, damping: 20, delay: 0.6 }} sx={{ display: 'flex', gap: 2 }}>
                        ...
                        <Box sx={{ p: { xs: 1.5, md: 3 }, pt: 0 }}>
                            <InputBox inputValue={inputValue} setInputValue={setInputValue} handleSend={handleSend} hasChat={hasChat} />
                        </Box>
                    </>
                ) : (
                    <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', p: { xs: 2, md: 3 } }}>
                        <motion.div
                            initial={{ scale: 0.8, opacity: 0, y: 20 }}
                            animate={{ scale: 1, opacity: 1, y: 0 }}
                            transition={{ type: 'spring', bounce: 0.5, duration: 0.8 }}
                        >
                            <Stack direction="row" alignItems="center" gap={{ xs: 1, md: 2 }} sx={{ mb: 4, justifyContent: 'center' }}>
                                <AutoAwesomeIcon sx={{ fontSize: { xs: 28, md: 40 }, color: 'primary.main' }} />
                                <Typography variant="h3" sx={{ fontFamily: '"Georgia", "Times New Roman", serif', fontWeight: 400, color: 'text.primary', fontSize: { xs: '1.75rem', md: '3rem' } }}>
